...
class ToolDrilling(Excellon, AppTool):
    ...
    def set_tool_ui(self):
        ...
        try:
            self.excellon_tools = deepcopy(self.excellon_obj.tools)
        except AttributeError:
            # no object loaded
            pass
        self.build_tool_ui()
    ...
    def build_tool_ui(self):
        ...
        order = self.ui.order_combo.get_value()
        if order == 1:  # 'fwd'
            sorted_items = sorted(self.excellon_tools.items(), key=lambda it: it[1]['tooldia'], reverse=False)
            new_tools = {}
            for idx, (_k, v) in enumerate(sorted_items, start=1):
                new_tools[idx] = v
            self.excellon_tools = new_tools
        elif order == 2:  # 'rev'
            sorted_items = sorted(self.excellon_tools.items(), key=lambda it: it[1]['tooldia'], reverse=True)
            new_tools = {}
            for idx, (_k, v) in enumerate(sorted_items, start=1):
                new_tools[idx] = v
            self.excellon_tools = new_tools
        ...
        if len(sel_rows) > 1:
            ...
            try:
                row_0 = list(sel_rows)[0]
                item_0 = self.ui.tools_table.item(row_0, 0)
                if item_0:
                    toolnr = int(item_0.text())
                    self.ui.tool_data_label.setText(
    ...
    def on_object_changed(self):
        ...
        if not hasattr(self, 'ui') or not self.ui or not hasattr(self.ui, 'object_combo'):
            return
        ...
        if self.excellon_obj is None:
            self.excellon_tools = {}
            if hasattr(self.ui, 'exc_param_frame') and self.ui.exc_param_frame:
                self.ui.exc_param_frame.setDisabled(True)
            # Defer set_tool_ui to avoid deleting the signal-emitting combo box
            QtCore.QTimer.singleShot(10, self.set_tool_ui)
            return
        else:
        ...
        if not sel_rows or len(sel_rows) == 0:
            if hasattr(self.ui, 'generate_cnc_button') and self.ui.generate_cnc_button:
                self.ui.generate_cnc_button.setDisabled(True)
            if hasattr(self.ui, 'tool_data_label') and self.ui.tool_data_label:
                self.ui.tool_data_label.setText(
    ...
    def on_tool_add_from_db_clicked(self):
        ...
        for idx in range(self.app.ui.plot_tab_area.count()):
            if self.app.ui.plot_tab_area.tabText(idx) == _("Tools Database"):
                ...
                break
        ret_val = self.app.on_tools_database(source='drilling')
        if ret_val == 'fail':
            return
    ...
    def on_drilling_tool_add_from_db_executed(self, tool):
        """
        ...
        if tool['data']['tool_target'] not in [0, 2]:   # [General, Drilling]
            for idx in range(self.app.ui.plot_tab_area.count()):
                if self.app.ui.plot_tab_area.tabText(idx) == _("Tools Database"):
        ...
        # we apply the tool data to all selected tools in the Drilling tool table
        selected_rows = []
        ...
        for row in selected_rows:
            tool_id = int(self.ui.tools_table.item(row, 0).text())
            # diameter stays the same as it is in Excellon file, but we update the parameters
            # or maybe the user wants to change diameter too? Standard behavior is to keep Excellon diameter
            # but use DB parameters.
            for d in tool_from_db['data']:
        ...
        self.app.inform.emit('[success] %s' % _("Tool from DB parameters applied."))
        self.build_tool_ui()
        ...
        if self.excellon_obj:
            ...
            for orig_tool, orig_tool_val in self.excellon_tools.items():
                ...
                for db_tool, db_tool_val in self.tools_db_dict.items():
                    ...
                    if targeted_tool not in [0, 2]:   # [General, Drilling]
                        continue

                    # if we find a tool with the same diameter in the Tools DB just update it's data
                    truncated_orig = self.app.dec_format(float(orig_tooldia), self.decimals)
                    truncated_db = self.app.dec_format(float(db_tooldia), self.decimals)
                    if truncated_orig == truncated_db:
                        tool_found += 1
    ...
    def update_ui(self):
        ...
        if len(sel_rows) == 1:
            # update the QLabel that shows for which Tool we have the parameters in the UI form
            try:
                item_id = self.ui.tools_table.item(list(sel_rows)[0], 0)
                if item_id:
                    tooluid = int(item_id.text())
                    self.ui.tool_data_label.setText(
...
class DrillingUI:
    ...
    def __init__(self, layout, app, name):
        ...
        self.addtool_from_db_btn.setToolTip(
            _("Open the Tools Database and pick a tool to apply its parameters\n"
              "to the selected tools in the Drilling Tool Table.")
        )

        tool_grid.addWidget(self.search_load_db_btn, 4, 0)
        tool_grid.addWidget(self.addtool_from_db_btn, 4, 1)
